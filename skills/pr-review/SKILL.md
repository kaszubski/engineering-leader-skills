...
2. **Read for intent and local context.** Establish the intended behavior and base branch. Read applicable repository instructions, then retrieve only the domain definitions, architectural decisions, and optional `TEAM-CONTEXT.md` entries relevant to this change. Follow their links selectively; do not load every document. If context is absent, proceed from observable code and state any material uncertainty. A documented tradeoff is evidence, not automatic immunity from a defect.
